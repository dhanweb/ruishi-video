      ...
      <view class="popup">
        <view class="popup-header">
          ...
          <view class="logined" v-else>
            <u--image shape="circle" :showLoading="true" :src="user.avatar" width="80px" height="80px" @click="goMine">
            </u--image>
        ...
        <view class="popup-list">
          <view class="item">
            <navigator url="/pages/index/index">
              <u-icon name="home" color="#fff" size="20" label="首页" labelSize="18" labelColor="#fff"></u-icon>
            </navigator>
          </view>
          <view class="item">
            <navigator url="/pages/list/list">
              <u-icon name="play-circle" color="#fff" size="20" label="视频列表" labelSize="18" labelColor="#fff"></u-icon>
            </navigator>
          </view>
          <view class="item">
            <navigator url="/pages/mine/mine">
              <u-icon name="account" color="#fff" size="20" label="个人中心" labelSize="18" labelColor="#fff"></u-icon>
            </navigator>
          </view>
          <view class="item">
            <!-- <text >后台管理</text> -->
            <!-- <navigator url="/pages/mine/mine"> -->
              <u-icon @click="goAdmin" name="setting" color="#fff" size="20" label="后台管理" labelSize="18" labelColor="#fff"></u-icon>
            <!-- </navigator> -->
          </view>
          <view class="item" v-if="isLogin">
            <text @click="handleLogout">
              <u-icon name="backspace" color="#fff" size="20" label="退出登录" labelSize="18" labelColor="#fff"></u-icon>
            </text>
          </view>
        </view>
      </view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goAdmin() {
        window.open('http://110.41.19.136:88')
      }
    },

    computed: {
      ...mapGetters(['user', 'category', 'isLogin']),
      color() {
        // if (this.placeholder) {
        //   return '#333';
        // } else {
...
<style lang="scss" scoped>
  .rs-header {
    ...
    .rs-header-box {
      ...
      box-shadow: 0 0 1px rgb(87, 81, 80);
      position: fixed;
      z-index: 999;
